...
puts open("03.in").count {|l| triangle?(l.scan(/\d+/).map(&:to_i)) }
...
rows = open("03.in").map {|l| l.scan(/\d+/).map(&:to_i) }

puts (rows.map{|e|e[0]} + rows.map{|e|e[1]} + rows.map{|e|e[2]})
  .each_slice(3)
  .count {|sides| triangle?(sides) }
