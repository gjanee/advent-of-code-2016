...
def real?(name, checksum)
  ...
  pairs = h.each_pair.to_a.sort {|l,r|
    l[1] == r[1] ? l[0] <=> r[0] : r[1] <=> l[1]
  }
  pairs[0,5].map(&:first).join == checksum
end

rooms = open("04.in").map {|l|
  m = /^([a-z-]*)-(\d+)\[([a-z]{5})\]$/.match(l)
  [m[1], m[2].to_i, m[3]]
}.select {|name,id,checksum| real?(name, checksum) }

puts rooms.map {|name,id,checksum| id }.reduce(:+)
...
# --------------------
#
# The other rooms have humorous, Easter-themed names, e.g., "military
# grade cryogenic bunny acquisition."
...
puts rooms.find {|name,id,checksum|
