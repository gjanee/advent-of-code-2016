...
puts open("07.in").count {|l|
  /^                    # from start of string
   (?!                  # negative lookahead:
     .*\[               #   a bracket in the distance
     [^\]]*             #   within bracketed section
     (\w)(?!\1)(\w)\2\1 #   ABBA
   )                    #
   .*                   # back to start, any intervening characters
   (\w)(?!\3)(\w)\4\3   # ABBA
  /x.match(l)
}
...
puts open("07.in").count {|l|
  /^                # from start of string
   (?:.*\])?        # or possibly starting after any bracketed section
   [^\[]*           # outside bracketed section
   (\w)(?!\1)(\w)\1 # ABA
   .*\[             # then, any bracket in the distance
   [^\]]*           # within bracketed section
   \2\1\2           # BAB
  /x.match(l) ||
  /^                # other way around this time
   .*\[             # find any bracketed section
   [^\]]*           # within bracketed section
   (\w)(?!\1)(\w)\1 # ABA
   .*\]             # go to end of any subsequent bracketed section
   [^\[]*           # outside bracketed section
   \2\1\2           # BAB
  /x.match(l)
}
