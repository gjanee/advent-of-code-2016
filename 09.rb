...
#   not treated any differently from the A that comes after it.  It
...
Input = open("09.in").read.gsub(/\s/, "")

def decompress_length(s, sublength_function)
  # sublength_function: function to call on substrings
  t = 0
  while true
    m = /\((\d+)x(\d+)\)/.match(s)
    break if m.nil?
    l, r = m[1].to_i, m[2].to_i
    t += m.pre_match.length
    t += sublength_function.call(s[m.end(0),l], sublength_function) * r
    s = s[m.end(0)+l..-1]
  end
  t += s.length
  t
...
puts decompress_length(Input, lambda {|s,_| s.length })
...
puts decompress_length(Input, method(:decompress_length))
