...
# --------------------
#
# Fortunately bots are well-behaved: the dependency graph is acyclic.
...
class Output
  ...
  def Output.[](id)
    @@all.member?(id) ? @@all[id] : Output.new(id)
  end
...
class Bot
  ...
  def Bot.[](id)
    @@all.member?(id) ? @@all[id] : Bot.new(id)
  end

  @@ready = [] # those bots that are ready to dispense their values

  def Bot.ready
    @@ready
  end

  attr_reader :id, :low, :high
  ...
  def set_rule(low_type, low_id, high_type, high_id)
    @low_dest = (low_type == "bot" ? Bot : Output)[low_id]
    @high_dest = (high_type == "bot" ? Bot : Output)[high_id]
  end

  def dispense
    @low_dest.accept(@low)
    @high_dest.accept(@high)
  end

end

open("10.in").each do |l|
  if /value (\d+) goes to bot (\d+)/.match(l)
    Bot[$2.to_i].accept($1.to_i)
  elsif /bot (\d+) gives low to (\w+) (\d+) and high to (\w+) (\d+)/.match(l)
    Bot[$1.to_i].set_rule($2, $3.to_i, $4, $5.to_i)
  end
end

while Bot.ready.length > 0
  Bot.ready.pop.dispense
end

puts Bot.values.find {|b| b.low == 17 && b.high == 61 }.id
