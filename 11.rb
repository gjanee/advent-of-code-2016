...
# they can only be powered by their corresponding RTG.  An RTG
# powering a microchip is still dangerous to other microchips.
#
...
# --------------------
#
# Breadth first search, but for decent performance either heuristics
# must be employed or branching must be limited.  We take the latter
# approach using a tip from another solver that is obvious in
# hindsight: there are many equivalent states.  A state consists of
# the locations of the items (along with the current floor, of
# course), but it is more convenient to think of a state as a list of
# pairs of locations (chip floor, generator floor), one per element.
# Since the elements are otherwise indistinguishable, any permutation
# of pairs is equivalent.  Thus given N elements, collapsing
# equivalent states reduces the branching by a factor of up to N!,
# depending on how many pairs are identical.  Collapsing is done
# simply by storing the pairs in sorted order.

require "set"

F = 4 # number of floors

class State

  attr_accessor :floor, :pairs

  def initialize(floor, pairs)
    @floor = floor
    @pairs = pairs
    normalize
  end
  ...
  def legal?
    @pairs.all? {|c_loc,g_loc|
      c_loc == g_loc || !@pairs.any? {|_,g| g == c_loc }
    }
  end

  def final?
    @pairs.all? {|c_loc,g_loc| c_loc == F-1 && g_loc == F-1 }
  end

  def move(floor, move_list)
    # move_list: list of indices [i,j] into @pairs
    s = State.new(floor, @pairs.map {|p| p.clone })
    move_list.each do |i,j|
      s.pairs[i][j] = floor
    end
    s.normalize
    s
  ...
  def items
    # returns items on current floor, expressed as move_list
    @pairs.each_with_index.flat_map {|p,i|
      c_loc, g_loc = p
      l = []
      l << [i,0] if c_loc == @floor
      l << [i,1] if g_loc == @floor
      l
    }
  end

  def eql?(other)
    @floor == other.floor && @pairs == other.pairs
  end
  def hash
    [@floor, @pairs].hash
  end
...
def solve(start)
  ...
  while true
    ...
    next_todo = []
    todo.each do |s|
      (s.items.combination(1).to_a + s.items.combination(2).to_a).each do |c|
        [s.floor-1, s.floor+1].select {|f| f >= 0 && f < F }.each do |f|
          ns = s.move(f, c)
          next if !ns.legal?
          return n if ns.final?
          if !seen.member?(ns)
            next_todo << ns
            seen.add(ns)
          end
...
elements = Hash.new {|h,k| h[k] = [nil, nil] }
# Assuming floors are described in order...
open("11.in").each_with_index do |l,f|
  l.scan(/(\w+)\s*(-compatible microchip|generator)/).each do |e,c_or_g|
    elements[e][c_or_g == "generator" ? 1 : 0] = f
  end
end
start = State.new(0, elements.values)
