...
def open?(x, y)
  ...
  ("%b" % (x*x+3*x+2*x*y+y+y*y+Input)).count("1") % 2 == 0
end
...
def bfs(&block)
  # Yields distances and locations to the associated block; returns
  # the first non-nil value returned by the block.
  frontier = [[1,1]]
  seen = Set.new
  seen.add([1,1])
  n = 0
  ...
  while true
    n += 1
    next_frontier = []
    frontier.each do |hx,hy|
      neighbors(hx, hy)
        .select {|x,y| open?(x, y) && !seen.member?([x,y])}
        .each do |x,y|
          v = yield(n, x, y)
          return v if !v.nil?
          next_frontier << [x,y]
