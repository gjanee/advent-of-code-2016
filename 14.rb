...
def hash(n)
  Digest::MD5.hexdigest("#{Salt}#{n}")
end

def solve
  a = (1+W).times.map {|n| hash(n) }
  n = 0
  k = 0
  while true
    if /(.)\1\1/.match(a[n])
      p = $1*5
      if (n+1..n+W).any? {|m| a[m].include?(p) }
        k += 1
        return n if k == 64
      end
    end
...
# --------------------
#
# This program takes 90s to run, 89.5s of which is computing hashes.
# So, no algorithmic optimization would be worthwhile.
