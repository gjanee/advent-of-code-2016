...
# generate appropriate random data, you'll need to use a modified
# dragon curve.
#
# Start with an appropriate initial state (your puzzle input).  Then,
# so long as you don't have enough data yet to fill the disk, repeat
# the following steps:
#
...
# is a 0.  This should produce a new string which is exactly half as
# long as the original.  If the length of the checksum is even, repeat
# the process until you end up with a checksum with an odd length.
#
...
#
# Combining all of these steps together, suppose you want to fill a
# disk of length 20 using an initial state of 10000:
#
...
# --------------------
#
# We encountered Ruby performance issues on this puzzle.  The natural
# functional way to compute the checksum of a string,
#
# while s.length%2 == 0
#   s = (0...s.length).step(2).map {|i| s[i] == s[i+1] ? "1" : "0" }.join
# end
#
# took 3x longer than an equivalent Python program.  Keeping the same
# functional expression but switching from strings to arrays of
# integers yielded no significant improvement.  The method below, in
# which we overwrite an array of integers, is comparable to Python.
# Even more disconcerting, though, trying the same overwrite method
# using strings (Ruby strings are mutable, after all) was 400x slower!

Input = "11100010111110100".chars.map(&:to_i)

def solve(n)
  s = Input.clone
  while s.length < n
    s = s + [0] + s.reverse.map {|v| 1-v }
  end
  l = n
  while l%2 == 0
    (0...l).step(2) do |i|
...
# the initial state in your puzzle input, what is the correct checksum
# for this disk?

puts solve(35651584).join
