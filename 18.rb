...
# you just triggered has run out of... whatever it tried to do to you.
...
# --------------------
#
# This is Wolfram cellular automaton rule 90
# <https://mathworld.wolfram.com/Rule90.html>, which generates
# Sierpinksi triangles.  Note that the center value is immaterial.

def solve(m)
  row = ".#{open("18.in").read.chomp}."
  n = 0
  m.times do
    n += row.count(".")-2
    r = (1..row.length-2).map {|i|
      (row[i-1] == "^")^(row[i+1] == "^") ? "^" : "."
    }.join
    row = ".#{r}."
  end
